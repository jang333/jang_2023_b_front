    ...
    객체 : 속성과 메소드를 가질 수 있는 모든것
        ...
        -기본자료형(숫자, 문자열, 불) 객체 아니다.
            1. 기본자료형 ---> 객체로 변환 (= 객체자료형,래핑 클래스)
                -??객체로 활용하려고 (속성/메소드를 사용할 수 있음 - 부가자료, 부가속성을 첨부할 수 있음)
            2.
                ...
                new Boolean(불자료형)
            3.문자열 타입[일시적 승급]
                -속성 추가는 할 수 없지만. js만든 사람이 제공하는 속성은 사용이 가능하다.
*/

//p.257
const a =[] //[ ] 대괄호를 사용했으므로 배열
...
console.log(typeof b); //함수는 객체의 특성을 완벽하게 가지고 있음. [js에서만]
...
console.log(f)  //273[x] -->Number {273}
f.sample=10
console.log(f) //Number {273, sample: 10} //객체는 키와값 = 속성
console.log(f.valueOf()); //객체명.valueOf();  //속성이 아닌 값 추출.

const g = Number(273);
    console.log(g);     //단순한 기본자료형 타입변환 [객체x]
...
const a2 = 12;
console.log('a2.power():', a2.power());     //n=2, this:12, return:144
console.log('a2.power(3):', a2.power(3));    //n=3, this:12, return:1728
console.log('a2.power(4):', a2.power(4));    //n=4, this:12, return:20736

//p.265
const h = '안녕하세요';
console.log(h.indexOf('안녕'))  //문자열내 '안녕'이 존재하면 앞문자의 인덱스로 추출
console.log(h.indexOf('하세'))  //2
console.log(h.indexOf('자바'))  //-1
    //vs
//console.log(h.contain('안녕'))  //h.contain is not a function [contain() 메소드 없다]
